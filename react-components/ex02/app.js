...
console.warn(
  `Folosind obiectul person si reduce, afiseaza in consola un
  string care contine skill-urile de pe pozitiile pare ale
...
const message1 = person.skills.reduce((message1, skill, index, skills) => {
  let punctuation = skill.length - 1 === index ? '' : ',';
...
const message3 = person.friends.reduce((message, friend, index, friends) => {
  ...
  const punctuation = friend.length - 1 === index ? '.' : ', ';
